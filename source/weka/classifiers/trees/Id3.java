 ...
 * 
 <!-- options-end -->
 *
 * @author Eibe Frank (eibe@cs.waikato.ac.nz), modified by Loc Nguyen 2009.12
...
public class Id3 
  ...
  protected String toString(int level) {
    ...
    return text.toString();
  	...
  	//added by Loc Nguyen 2010.02.01
  	protected String toHtml(int level, CourseInfo courseInfo) {
  		...
  		if (m_Attribute == null) {
  			if (Instance.isMissingValue(m_ClassValue)) {
  				...
  				String cName = m_ClassAttribute.value((int) m_ClassValue);
  				if(courseInfo != null) {
  					cName = courseInfo.conceptsInfoTbl.getConceptInfo(cName).getTitle();
  					if(cName == null || cName.trim().length() == 0)
  						cName = m_ClassAttribute.value((int) m_ClassValue);
  				}
  				text.append(": <font color=\"FF0000\">" + cName + "</font>");
  			} 
  		}
  		else {
  			for (int j = 0; j < m_Attribute.numValues(); j++) {
  				...
  				text.append("<font color=\"0000FF\">" + m_Attribute.name() + " = " + m_Attribute.value(j) + "</font>");
  				text.append(m_Successors[j].toHtml(level + 1, courseInfo));
  			}
  		}
  		return text.toString();
  	}
