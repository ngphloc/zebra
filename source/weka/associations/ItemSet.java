...
public class ItemSet
  ...
  public String toString(Instances instances) {
    ...
    return text.toString();
  	...
  	//@loc Nguyen add
  	public String toString2(Instances instances) {
  		StringBuffer text = new StringBuffer();
  	...
  	//Loc Nguyen add
  	public ArrayList<String> getItemNames(Instances instances) {
  		ArrayList<String> names = new ArrayList<String>();
  		for (int i = 0; i < instances.numAttributes(); i++)
  			if (m_items[i] != -1) {
  				names.add(instances.attribute(i).value(m_items[i]));
	        }
  		return names;
  	}
  ...
  public double getSupport() {
	  if(m_totalTransactions == 0) return 0;
	  return (double)m_counter / (double)m_totalTransactions;
  }
}
