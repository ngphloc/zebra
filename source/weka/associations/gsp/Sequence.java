 ...
 * 
 * @author  Sebastian Beer, modified by Loc Nguyen 2008.04
...
public class Sequence
  ...
  /**
   * Checks if whether this sequences contains other sequence.
   * 
   * @return 			true, if containing
   */
  	public boolean contains(Sequence seq2) {
  		int idx = 0;
  		for (int j = 0; j < seq2.m_Elements.size(); j++) {
  			Element seq2Element = (Element) seq2.m_Elements.elementAt(j);
  			boolean found = false;
  			int i = 0;
  	  		for (i = idx; i < m_Elements.size(); i++) {
  	  			Element thisElement = (Element) m_Elements.elementAt(i);
  	  			if (thisElement.contains(seq2Element)) {
  	  				found = true;
  	  				break;
  	  			}
  			}
  ...
  public int length() {
	  int length = 0;
	  for(int i = 0; i < m_Elements.size(); i++) {
		  length += ( (Element)m_Elements.elementAt(i) ).length();
	  }
	  return length;
  }
}
