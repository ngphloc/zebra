 ...
 * 
 * @author  Sebastian Beer, modified by Loc Nguyen 2009.02
...
public class Element
  ...
  public boolean equals(Object obj) {
    ...
    return true;
  	...
  	public boolean contains(Object obj) {
  		...
  		for (int j=0; j < element2.m_Events.length; j++) {
  			if(element2.m_Events[j] == -1) continue;
  			boolean found = false;
  	  		for (int i=0; i < m_Events.length; i++) {
  	  			if (m_Events[i] == element2.m_Events[j]) {
  	  				found = true;
  	  				break;
  	  			}
  	  		}
  ...
  public String nomalizeString(Instances dataSet) {
	    StringBuffer result = new StringBuffer();
	    int addedValues = 0;
	    ...
	    for (int i=0; i < m_Events.length; i++) {
	    	if (m_Events[i] > -1) {			
		    	String value = dataSet.attribute(i).value(m_Events[i]);
		    	if(result.indexOf(value) > 0) continue;
	    	  result.append(value + ",");
	    	  addedValues++;
	      }
	    }
