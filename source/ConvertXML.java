...
*/
/**
 * ConvertXML.java 1.0 August 30, 2008
 *
 ...
 */
/*
 * Servlet that generates the interface to change from the xml format to the internal format.
 */
...
public class ConvertXML extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (request.getParameter("coursename")==null) {
            ...
            " <form method=\"POST\" action=\"ConvertXML\">\n"+
            " <h2>Concept List Conversion</h2><table><tr>\n"+
            " <tr><td>Enter authorname: </td>"+
            " <td>\n<input type=\"text\" name=\"authorname\" size=30></td>\n"+
            " <td><input type=\"submit\" name=\"convertsubmit\" value=\"OK\"></td></tr>\n"+
            " <tr><td>and the coursename to convert: </td>"+
            ...
            try {
                ...
                boolean created = (new ConceptList()).getConceptList(fileName, concept);
                if (created) {
                    if (!KoenFormat.getMessage().equals("")) response.getWriter().write(" <b>"+KoenFormat.getMessage()+"</b>\n");
                        else {
                            response.getWriter().write(" <h2>Concept List</h2>\nThe concept list has been <b>succesfully converted</b> from an XML file to the internal format.\n");
                            System.out.println(KoenFormat.getMessage());
                        }
                } else response.getWriter().write(" <h2>Warning!</h2>\nThere was an <b>error</b> converting the concept list from an XML file to the internal format.\nTry to repeat it after restarting the server.\n");
            } catch(Exception e) {
                System.out.println(e);
                response.getWriter().write(" <h2>Warning!</h2>\nThere was an <b>exception</b> converting the concept list from an XML file to the internal format.\n");
            }
        }
