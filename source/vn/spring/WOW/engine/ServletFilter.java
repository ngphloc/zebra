...
*/
/**
 * ServletFilter.java 1.0, June 1, 2008
 *
...
public class ServletFilter {
	...
	public ServletFilter(Profile profile, Node node, String course) {
	...
	public StringBuffer Filter(Node node) throws InvalidAttributeException {
		...
		node.getChildNodes();
		StringBuffer servlet = getText(node);
		String href = WOWStatic.config.Get("CONTEXTPATH").trim() + "/" + servlet.toString().trim();
	...
	public StringBuffer getText(Node node) {
		...
		for (int i=0; i<nodelist.getLength(); i++) {
			...
			if (child.getNodeName().equals("servletname")) {
				buffer = getText(child);
			}
			if (child.getNodeName().equals("linkdescription")) {
				LastChild = child;
			}
			if (child.getNodeName().equals("logoff")) {
				buffer.append((new ServletNameFilter(child).Filter(child)).toString());
			}
