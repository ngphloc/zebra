...
/**
 * Use Walley generalization of the Choquet lower and upper    
 * integrals to obtain the lower and upper expectations of     
...
public class GeneralizedChoquetIntegral {
  ...
  /**
   * Calculate the lower and upper Choquet integrals using     
   * Walley's generalization, for a total                      
   * variation neighborhood.                                   
   */
  public GeneralizedChoquetIntegral(TwoMonotoneCapacity tmc, DiscreteFunction df) {
      ...
      for (i=1; i<=df_positive.length; i++) {
        df_positive[i-1] = ((Double)(positive.elementAt(i))).doubleValue() -
      ...
      for (i=1; i<=df_negative.length; i++) {
        df_negative[i-1] = ((Double)(negative.elementAt(i))).doubleValue() -
  ...
  /**
   * Collect the positive values in df and sort them in        
   * increasing order (first value is assumed zero).           
   */
  private Vector sort_positive(DiscreteFunction df) {
    ...
    for (int i=0; i<df.number_values(); i++) {
        ...
        for (int j=0; j<sorted.size(); j++) {
            if (df.get_value(i) < ((Double)sorted.elementAt(j)).doubleValue()) {
  ...
  /**
   * Collect the negative values in df and sort them in     
   * decreasing order (first value is assumed zero).        
   */
  ...
  /**
   * Obtain the lower and upper probability for the event      
   *  { df(x) > sorted_value[i] }                              
   */
  private void bound_positive(TwoMonotoneCapacity tmc,
                              DiscreteFunction df, Vector sorted_values,
