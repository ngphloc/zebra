 ...
 */
package vn.spring.bayes;
import vn.spring.bayes.JavaBayesInterface.*;
import vn.spring.bayes.InferenceGraphs.*;

import java.awt.*;
import java.applet.Applet;

public class JavaBayes extends Applet implements JavaBayesI {
  ...
  public static void main(String argv[]) {
    ...
    if (argv.length > 0) {
        jb.construct(false, argv[0]);
        String filename = argv[0];
        System.out.println(filename);
        jb.open(filename);
    } else
        jb.construct(false, null);
  }
   ...
   */
  private void construct(boolean ia, String filename) {
    is_applet = ia;
    ...
    if (filename == null)
        JavaBayesHelpMessages.show(JavaBayesHelpMessages.start_scratch_message);
    else 
        JavaBayesHelpMessages.show(JavaBayesHelpMessages.start_file_message);
  }
