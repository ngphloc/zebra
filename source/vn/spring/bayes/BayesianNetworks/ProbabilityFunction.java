...
public class ProbabilityFunction extends DiscreteFunction {
   ...
   * which specifies a value of the function.
   */
  ...
  /**
   * Evaluate a function given a (possibly
   * partial) instantiation of variables through the markers.
   * The markers indicate which variables are present in the
   ...
   * The current implementation is very limited; it assumes that
   * both the ProbabilityFunction object and the
   * DiscreteFunctions object has a single variable, and the
   * variable must be the same for both functions.
   */
   ...
   * This assumes that the probability values are unnormalized,
   * equal to p(x, e) where e is the evidence.
   * The current implementation is very limited; it assumes that
   * both the ProbabilityFunction object and the
   * DiscreteFunctions object has a single variable, and the
   * variable must be the same for both functions.
