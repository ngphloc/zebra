...
package vn.spring.bayes.QuasiBayesianNetworks;
...
public class QBConvertInterchangeFormat extends ConvertInterchangeFormat {
   ...
   * ProbabilityFunction object out of the definition         *
   * found by the parser and the information contained in     *
   * the BayesNet object; the ProbabilityFunction object may  *
   * in fact be a Quasi-Bayesian model.                       *
   ************************************************************/
  ...
  /************************************************************
   * Fill the values with the contents of the tables          *
   * in the upf object.                                       *
   ************************************************************/
   ...
   * specification of defaults in the upf object.             *
   ************************************************************/
	...
	if (extreme_points != null) {
    	...
    	for (j=0; j<extreme_points.length; j++) {
        	for (i=0; i<extreme_points[j].length; i++)
	        	if (extreme_points[j][i] == -1.0) extreme_points[j][i] = 0.0;
        }
    }
    else { // Second case: single distribution; just fill zeros where needed
       	for (i=0; i<values.length; i++)
